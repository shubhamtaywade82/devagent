    ...
    def self.default
      new([
            ...
            Tool.new(
              name: "fs.create",
              category: "filesystem",
              description: "Create a new file with provided initial content (logical only).",
              purpose: "Safely add new files under repo control (controller applies as a diff).",
              when_to_use: [
                "When adding a new file (e.g., a new Ruby class/module, spec file, or script)",
                "When scaffolding a small standalone Ruby program inside the repo"
              ],
              when_not_to_use: [
                "To overwrite existing files (use fs.write)",
                "To create many large files in one step"
              ],
              inputs_schema: {
                "type" => "object",
                "required" => %w[path content],
                "properties" => {
                  "path" => { "type" => "string", "description" => "Relative path to new file inside repo" },
                  "content" => { "type" => "string", "description" => "Initial full file contents" }
                }
              },
              outputs_schema: {
                "type" => "object",
              ...
              examples: {
                "valid" => [
                  {
                    "input" => { "path" => "tmp/hello.rb", "content" => "puts 'hello'\n" },
                    "output" => { "proposed_change" => "--- /dev/null\n+++ b/tmp/hello.rb\n@@ ..." }
                  }
                ],
                "invalid" => [
                  { "input" => { "path" => "tmp/hello.rb" }, "reason" => "Missing required content" }
                ]
              },
              handler: nil, # logical-only: controller translates to internal fs.write_diff (add-file diff)
              internal: false,
              visible_phases: %i[planning]
            ),
            Tool.new(
              ...
              inputs_schema: {
                ...
                "properties" => {
                  ...
                  "args" => { "type" => "array", "items" => { "type" => "string" },
                              "description" => "Arguments array for program invocation" },
                  "accepted_exit_codes" => { "type" => %w[array null], "items" => { "type" => "integer" },
                                             "description" => "Non-zero exit codes that are acceptable for this step" },
                  "allow_failure" => { "type" => %w[boolean null],
                                       "description" => "If true, do not fail the step on non-zero exit" }
                },
                "required" => %w[program args]
              },
              outputs_schema: {
                "type" => "object",
              ...
              examples: {
                "valid" => [
                  {
                    "input" => { "program" => "bundle", "args" => %w[exec rspec] },
                ...
                "valid" => [
                  {
                    ...
                    "output" => { "root_cause" => "Foo constant is missing or not required/loaded",
                                  "confidence" => 0.7 }
                  }
                ],
                "invalid" => [
