  ...
  class CLI < Thor
    ...
    def diag
      ...
      rows = [
        ...
        ["Models", info[:models].map { |k, v| "#{k}=#{v}" }.join(", ")],
        ["Embedding", info[:embedding].map { |k, v| "#{k}=#{v}" }.join(", ")],
        ["OPENAI_API_KEY", info[:openai_key]]
      ]
      UI::Table.new(header: %w[Key Value], rows: rows, output: $stdout).render
      info
    end
