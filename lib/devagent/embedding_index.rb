...
module Devagent
  ...
  class EmbeddingIndex
    ...
    def build!
      ...
      if embeddings.any?
        vector_length = Array(vectors.first).size
        clear_on_dimension_mismatch!(vector_length)
        store.upsert_many(embeddings.map { |entry| serialize(entry) })
        persist_meta(current_backend.merge("dim" => vector_length))
      end
      embeddings
    end
    ...
    def clear_on_dimension_mismatch!(new_dim)
      saved_dim = metadata["dim"]
      return if saved_dim.nil? || saved_dim.to_i == new_dim.to_i

      logger.call("Embedding dimension changed (#{saved_dim} -> #{new_dim}). Resetting index…")
      store.clear!
    end
