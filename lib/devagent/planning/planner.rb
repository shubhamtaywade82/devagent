      ...
      def build_prompt(user_prompt, is_empty, retrieved_files: [])
        ...
        <<~PROMPT
          ...
          - Language-specific linters (ONLY include if you're confident the tools are available, and ALWAYS use allow_failure: true):
            ...
            * JavaScript/TypeScript files (.js, .jsx, .ts, .tsx): Use "npm run lint" or "eslint" to check, "npm run lint:fix" or "eslint --fix" to auto-fix (only if package.json exists). Set 'allow_failure: true' for linting steps.
          ...
          - Example linting step with allow_failure: {"step_id": 2, "action": "exec.run", "command": "python3 -m flake8 playground/file.py", "accepted_exit_codes": [0, 1], "allow_failure": true, "reason": "Check code style (optional)", "depends_on": []}
